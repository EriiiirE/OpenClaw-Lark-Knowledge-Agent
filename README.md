...
端到端个人知识库 Agent 项目（采集 -> 清洗 -> RAG -> 渠道接入）的源码展示仓库。

## What Is Included

- `projects/Celueshi`: 微信公众号目录页内容抓取与 Markdown 归档（Playwright）
- `projects/Jingyingrike`: 得到课程内容抓取与专题化归档（Playwright）
- `projects/PersonalKnowledgeAI`: 本地知识库标准化、分类、切块、混合检索与问答（BM25 + Vector + RAG）

## OpenClaw / Feishu Integration

本仓库不内置 OpenClaw 源码（避免体积过大、便于独立维护）。

接入方式：

1. 安装 OpenClaw（官方）
2. 安装 Feishu 插件 `@openclaw/feishu`
3. 将本仓库 `PersonalKnowledgeAI` 作为本地知识服务（CLI/Streamlit）
4. 通过 OpenClaw 路由把飞书消息接入你的个人 Agent 流程
